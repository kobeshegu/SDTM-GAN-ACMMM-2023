...
def random_mask(img, mask_ratio, mask_size):
    ...
    mask[:, :len_keep] = 0
    mask = torch.gather(mask, dim=1, index=ids_restore).reshape(N, 1, 4, 4)
...
class Discriminator(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if self.laplace:
            ...
            logit_adv_lap = self.cnn_adv_lap(feat_lap).view(B * K, -1)
        feat = self.cnn_f(x)
        # frequency analysis
        logit_fre = None
        if self.fre_loss:
...
class Generator(nn.Module):
    ...
    def forward(self, xs):
        ...
        fake_x = self.decoder(feat_gen, skips, self.wavegan, self.wavegan_mean, base_index)
...
class Decoder(nn.Module):
    ...
    def forward(self, x, skips, wavegan, wavegan_mean, base_index):
        x1 = self.Upsample(x)
        x2 = self.Conv1(x1)
        if wavegan:
